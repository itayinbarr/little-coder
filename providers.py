"""
Multi-provider support for little-coder.
...
PROVIDERS: dict[str, dict] = {
    ...
    "ollama": {
        ...
        "models": [
            ...
            "qwen2.5-coder", "deepseek-r1", "gemma3", "gemma4",
        ],
    },
    "lmstudio": {
        "type":       "openai",
...
_TOOL_FALLBACK_WARNED: dict[str, bool] = {}  # track per-model warning
...
def stream_ollama(
    ...
    # Use model profile context limit if available, else config, else default
    from local.config import get_model_profile, is_small_model
    _profile = get_model_profile(model) if is_small_model(model) else {}
    _ctx = _profile.get("context_limit") or config.get("context_limit", 128000)
    _temp = _profile.get("temperature", config.get("temperature"))

    payload = {
        "model": model,
        "messages": oai_messages,
        "stream": True,
        "keep_alive": -1,  # prevent model unload during long tool executions
        "options": {
            "num_ctx": _ctx
        }
    }
    if _temp is not None:
        payload["options"]["temperature"] = _temp

    # Thinking control for reasoning models.
    # Ollama uses top-level "think": true/false (NOT inside options).
    # Ollama has NO native thinking budget cap (ollama/ollama#10925).
    # We enforce a budget by counting thinking tokens in the stream and
    # aborting + retrying with think=false if the budget is exceeded.
    _think_budget = _profile.get("thinking_budget", 0)
    if _think_budget > 0:
        payload["think"] = True
    else:
        payload["think"] = False

    # Decide tool-calling strategy: text-based vs native
    _use_text_tools = (
        model in _TOOL_FALLBACK_WARNED
        or _profile.get("prefer_text_tools", False)
    )

    if _use_text_tools and tool_schemas and not config.get("no_tools"):
        from local.output_parser import inject_tool_instructions
        for msg in oai_messages:
            if msg.get("role") == "system":
                msg["content"] = inject_tool_instructions(msg["content"], tool_schemas)
                break
    elif tool_schemas and not config.get("no_tools"):
    ...
    # Timeout for Ollama requests: prevents infinite hangs from deep thinking chains.
    # Ollama streams tokens, so this is a *connection* timeout — once streaming starts
    # the socket stays alive.  600s (10 min) is generous for first-token latency.
    _ollama_timeout = config.get("ollama_timeout", 600)

    try:
        resp_cm = urllib.request.urlopen(req, timeout=_ollama_timeout)
    except urllib.error.HTTPError as e:
        if e.code in (400, 500) and "tools" in payload:
            # Model doesn't support native tool calling — retry with text-based fallback
            if not _TOOL_FALLBACK_WARNED.get(model):
                _TOOL_FALLBACK_WARNED[model] = True
                print(
                    f"\n\033[33m[info] {model}: no native tool-calling support."
                    " Using text-based tool instructions.\033[0m"
                )
            payload.pop("tools", None)
            # Inject text-based tool instructions into system prompt
            from local.output_parser import inject_tool_instructions
    ...
    _thinking_exceeded = False
    _thinking_token_count = 0
    _thinking_text = ""  # accumulate thinking for reuse on budget-exceeded retry
    ...
    with resp_cm as resp:
        for line in resp:
            ...
            if "thinking" in msg and msg["thinking"]:
                chunk = msg["thinking"]
                _thinking_token_count += max(1, len(chunk) // 4)  # rough estimate
                _thinking_text += chunk
                yield ThinkingChunk(chunk)

                # Check thinking budget — abort if exceeded
                if _think_budget > 0 and _thinking_token_count > _think_budget:
                    _thinking_exceeded = True
                    break
    ...
    # If thinking budget was exceeded, retry with thinking off but inject
    # the partial reasoning as an assistant message so it's not wasted.
    if _thinking_exceeded:
        import sys
        print(
            f"\n\033[33m[info] Thinking budget exceeded ({_thinking_token_count} > {_think_budget})."
            " Retrying without thinking.\033[0m",
            file=sys.stderr,
        )
        text = ""
        tool_buf.clear()
        ...
        # Inject truncated thinking as context: add an assistant message with the
        # partial reasoning, then a user nudge to continue without over-thinking.
        if _thinking_text.strip():
            # Trim to a reasonable size (last 2000 chars to stay within context)
            trimmed = _thinking_text.strip()[-2000:]
            payload["messages"].append({
                "role": "assistant",
                "content": f"Here is my analysis so far:\n{trimmed}",
            })
            payload["messages"].append({
                "role": "user",
                "content": "Good analysis. Now produce the final answer directly — no more deliberation needed.",
            })

        req = _make_request(payload)
        with urllib.request.urlopen(req, timeout=_ollama_timeout) as resp2:
            for line in resp2:
                if not line.strip(): continue
                try:
                    data = json.loads(line)
    ...
    # but we can do a rough estimate or 0, little-coder handles zero gracefully
